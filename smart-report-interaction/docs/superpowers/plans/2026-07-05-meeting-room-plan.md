...
**Goal:** 将签到、汇报、互动、分析融合为统一的腾讯会议风格主屏 `MeetingRoom.vue`。

**Architecture:** 单文件三栏布局，顶部会议信息、左栏议程+签到、中栏汇报、右栏互动、底部功能栏。业务逻辑从旧页面迁移。路由重定向保留兼容。

**Tech Stack:** Vue 3 + Element Plus + ECharts，复用现有 API 和 Store。

---

### Task 1: 创建 MeetingRoom.vue

**Files:**
- Create: `smart-report-interaction/frontend/src/views/MeetingRoom.vue`

- [ ] **Step 1: 创建文件骨架**

新建 `smart-report-interaction/frontend/src/views/MeetingRoom.vue`，包含三栏布局模板、所有业务逻辑脚本、样式。

完整代码如下（单文件含 template + script + style）：
...
const loadSign = async () => {
  ...
  try {
    const a = await getMeetingAnalytics(meeting.id)
    meetingQuality.value = a.data
  } catch {}
}
...
const saveSpeech = async () => {
  ...
  try {
    await apiSave({ meetingId: meeting.id, speakerId, content: speechContent.value })
...
const sendInter = async () => {
  ...
  try {
    await sendMessage({ meetingId: meeting.id, userId: uid, content: interContent.value, interactType: interType.value })
...
watch(showQR, async (v) => {
  if (v) {
    await nextTick()
    if (qrCanvas.value) {
      await QRCode.toCanvas(qrCanvas.value, `${location.origin}/meeting/${meeting.id}`, { width: 200, margin: 1 })
    }
  }
})
...
```bash
git add smart-report-interaction/frontend/src/views/MeetingRoom.vue
git commit -m "feat: add unified meeting room main screen"
```
...
```bash
git add smart-report-interaction/frontend/src/router/index.js
git commit -m "feat: add /meeting/:id route, redirect old routes"
```
...
打开 `http://localhost:5174/meeting/1`，确认三栏布局渲染、签到/汇报/互动功能正常。

- [ ] **Step 3: 验证旧路由重定向**

打开 `http://localhost:5174/sign`、`/report`、`/interaction`，确认自动跳转到 `/meeting/1`。

- [ ] **Step 4: 提交**

```bash
git add smart-report-interaction/frontend/
git commit -m "chore: integration verification for meeting room"
```
